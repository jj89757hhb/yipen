...
@interface SendActiviyViewController ()<UITableViewDataSource,UITableViewDelegate,ZYQAssetPickerControllerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,UITextViewDelegate>
@property(nonatomic,strong)NSMutableArray *imgList;
@property(nonatomic,strong)SendActivityTableViewCell *sendActivityCell;
@end

@implementation SendActiviyViewController
static NSString *identify=@"identify";
//static NSInteger Max_Pic=9;
- (void)viewDidLoad {
    [super viewDidLoad];
...
//发布
-(void)sendAction{
    if ([_sendActivityCell.titleTF.text length]==0) {
        [SVProgressHUD showErrorWithStatus:@"请输入标题"];
        return;
    }
    if (([_sendActivityCell.contentTV.text length]==0)) {
    ...
    NSDictionary *dic=@{@"Uid":[DataSource sharedDataSource].userInfo.ID,
                        @"Title":_sendActivityCell.titleTF.text,
                        @"Message":_sendActivityCell.contentTV.text,
                        @"Type":[NSNumber numberWithInteger:1],
                        @"CityId":@"2"
                        
                        };
    [HttpConnection postSameCityInfos:dic pics:_imgList WithBlock:^(id response, NSError *error) {
        if (!error) {
...
-(void)initTable{
    ...
    [myTable registerNib:[UINib nibWithNibName:@"SendActivityTableViewCell" bundle:nil] forCellReuseIdentifier:identify];
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        SendActivityTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:identify forIndexPath:indexPath];
        cell.selectionStyle=UITableViewCellSelectionStyleNone;
        cell.contentTV.delegate=self;
        self.sendActivityCell=cell;
        return cell;
    }
    else{
...
-(void)textViewDidChange:(UITextView *)textView
{
    if ([textView.text length]) {
        [_sendActivityCell.popL setHidden:YES];
    }
    else{
        [_sendActivityCell.popL setHidden:NO];
    }
}
