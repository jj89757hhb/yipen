...
//        label2.textAlignment=NSTextAlignmentCenter;
    ...
    priceTF=[[UITextField alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(label1.frame)+25, bgView.frame.size.width-15*2, 35)];
    priceTF.borderStyle=UITextBorderStyleRoundedRect;
    priceTF.placeholder=@"请输入您的报价";
    priceTF.keyboardType=UIKeyboardTypeNumberPad;
    [priceTF becomeFirstResponder];
...
//    [surePrice addTarget:self action:@selector(surePriceAction) forControlEvents:UIControlEventTouchUpInside];
    ...
    float btn_height=38;
    UIButton *cancelBtn=[[UIButton alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(priceTF.frame)+15, bgView.frame.size.width/2, btn_height)];
    [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    [cancelBtn setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
    UIButton *addPriceBtn=[[UIButton alloc] initWithFrame:CGRectMake(bgView.frame.size.width/2, CGRectGetMaxY(priceTF.frame)+15, bgView.frame.size.width/2, btn_height)];
    [addPriceBtn addTarget:self action:@selector(baoJiaAction) forControlEvents:UIControlEventTouchUpInside];
    [addPriceBtn setTitle:@"确定" forState:UIControlStateNormal];
    [addPriceBtn setTitleColor:BLUECOLOR forState:UIControlStateNormal];
    UIView *line1=[[UIView alloc] initWithFrame:CGRectMake(0, cancelBtn.frame.origin.y, bgView.frame.size.width, 0.5)];
    UIView *line2=[[UIView alloc] initWithFrame:CGRectMake(bgView.frame.size.width/2, cancelBtn.frame.origin.y, 0.5, btn_height)];
    line1.backgroundColor=Line_Color;
    line2.backgroundColor=Line_Color;
...
-(void)surePriceAction{
    if (_offerPriceBlock&&[priceTF.text integerValue]>0) {
...
-(void)baoJiaAction{
    ...
    if (_baoJiaBlock&&[priceTF.text integerValue]>0) {
        _baoJiaBlock(priceTF.text);
    }
}
