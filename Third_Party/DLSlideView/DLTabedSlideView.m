...
- (void)layoutBarAndSlide{
    ...
    slideView_.frame = CGRectMake(0, self.tabbarHeight+self.tabbarBottomSpacing, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)-self.tabbarHeight-self.tabbarBottomSpacing-44-20);
    NSLog(@"slideView_%@",[slideView_ description]);
...
- (UIViewController *)DLSlideView:(DLSlideView *)sender controllerAt:(NSInteger)index{
    ...
    if ([ctrlCache_ objectForKey:key]) {
        ...
        UIViewController *ctrl = [self.delegate DLTabedSlideView:self controllerAt:index];//创建controller对象
