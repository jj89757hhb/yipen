...
//    [self addSubview:[[[NSBundle mainBundle] loadNibNamed:@"HZAreaPickerView" owner:self options:nil] objectAtIndex:0]];
    if (self) {
...
//    [quitBtn setBackgroundImage:[UIImage imageNamed:@"quit"] forState:UIControlStateNormal];
    ...
    [quitBtn addTarget:self action:@selector(cancelAction) forControlEvents:UIControlEventTouchUpInside];
    
//    [quitBtn setTintColor:DEEPORANGECOLOR];
    [quitBtn setTitleColor:BLACKCOLOR forState:UIControlStateNormal];
    [quitBtn setTitle:@"取消" forState:UIControlStateNormal];
    [dateToolBar addSubview:quitBtn];
    
    UIButton *finishBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    //    [quitBtn setBackgroundImage:[UIImage imageNamed:@"quit"] forState:UIControlStateNormal];
    finishBtn.frame = CGRectMake(SCREEN_WIDTH-55-15, 10, 55, 25);
    [finishBtn addTarget:self action:@selector(finishAction) forControlEvents:UIControlEventTouchUpInside];
    
    //    [quitBtn setTintColor:DEEPORANGECOLOR];
    [finishBtn setTitleColor:BLACKCOLOR forState:UIControlStateNormal];
    [finishBtn setTitle:@"完成" forState:UIControlStateNormal];
    [dateToolBar addSubview:finishBtn];
    
    
    self.frame = CGRectMake(0, view.frame.size.height+44, SCREEN_WIDTH, self.frame.size.height);
...
-(void)finishAction{
    ...
    self.locatePicker.delegate = nil;
    self.locatePicker = nil;
    if ([self.delegate respondsToSelector:@selector(pickerFinish:)]) {
        [self.delegate pickerFinish:self];
    }
